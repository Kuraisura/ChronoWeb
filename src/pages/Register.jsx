...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (invalidEmail) return setError(invalidEmail);
    ...
    try {
      ...
      if (data.session) window.location.href = safeReturnTo();
  ...
  const handleVerify = async () => {
    setError("");
    if (!/^\d{6}$/.test(otpCode)) return setError('Enter the complete 6-digit verification code.');
    setLoading(true);
    try {
